<template>
  ...
  >
    <div class="d-flex justify-space-between align-center">
      ...
      >
      <div
        v-if="$vuetify.breakpoint.lgOnly && ordersLength > 0"
        style="width: 250px"
      >
        <v-text-field
          v-model="searchId"
    ...
    <div
      v-if="!showOnOrders || showOrders.length === 0"
      class="border mt-6 h-full d-flex align-center justify-center h-min"
    >
      <v-progress-circular
        v-if="!showOnOrders"
        color="grey"
    ...
    <div v-else class="pb-2 mt-7 h-full">
      ...
      <div v-for="(e, i) in renderOrders" :key="i" class="e-panel py-3">
          ...
          <div class="d-flex _resumen">
            ...
            <v-col cols="3" class="text-capitalize">
              <span>
                {{
                  `${e.remitente.name} ${
                    e.remitente.nameSecond ? e.remitente.nameSecond : ''
                  } ${e.remitente.surname} ${
                    e.remitente.surnameSecond ? e.remitente.surnameSecond : ''
                  }`
                }}
              </span>
            </v-col>
            ...
            <v-col cols="2" class="text-lg-right">
              ...
              <span> {{ Number($unFormat(`${e.amount.pesos}`)).format(0, 3, '.') }} $ </span>
            </v-col>
            ...
            <v-col cols="2" class="text-lg-right">
              <strong> {{ calcPercentage($unFormat(`${e.amount.pesos}`)) }} $</strong>
            </v-col>
...
export default {
  ...
  computed: {
    ...
    filterByIdDestinatario() {
      return this.showOrders.filter((o) =>
        o.destinatario.id.match(this.searchId)
      )
    },
    renderOrders() {
      if (this.showOrders) {
