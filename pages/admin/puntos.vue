<template>
  <v-card class="pa-6 b-shadow">
    <div class="d-flex flex-column flex-lg-row justify-space-between align-center mb-4">
      ...
      <div>
        <v-chip style="height: 28px;" color="green" label text-color="white">
          ...
          {{ showUsers ? showUsers.length : 0 }} Registrados
        </v-chip>
    ...
    <v-pagination
      v-if="perPage < showUsers.length"
      v-model="page"
      :length="Math.ceil(this.showUsers.length / this.perPage)"
      :total-visible="7"
      color="secondary"
...
export default {
  ...
  methods: {
    paginate() {
      ...
      this.paginationChunks = this.showUsers.slice(from, to)
    },
