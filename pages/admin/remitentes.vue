<template>
  <v-card class="pa-6 b-shadow">
    ...
    >
      ...
      >
      <div>
        <v-chip style="height: 28px" color="green" label text-color="white">
          <v-icon left> mdi-label </v-icon>
          {{ $currencyWithoutComma(`${remitentesLength}`) }} Registrados
        </v-chip>
    ...
    <div class="d-flex justify-end">
      <form @submit.prevent="searchById">
        <div class="d-flex align-center flex-wrap">
          ...
          <v-btn class="ml-2 mb-2" type="submit" color="indigo" text-color="white">
            <v-icon color="white"> mdi-magnify </v-icon> <span style="color:white">BUSCAR</span>
          </v-btn>
    ...
    <div class="mb-3">
      <v-chip color="indigo" label text-color="white">
        <v-icon left> mdi-label </v-icon>
        Resultado: {{ showSearchResults ? searchResults.length : 0 }}
      </v-chip>
    </div>
    ...
    <!-- <v-pagination
      v-if="perPage <= remitentesLength && showSearchResults"
      v-model="page"
