    ...
    >
      <v-card v-if="order">
        <v-toolbar dark color="purple">
          <v-toolbar-title>
            {{ resume ? 'Administrar la' : 'Detalles de la' }} orden
        ...
        <v-container class="mt-7">
          ...
          <div class="d-flex flex-column align-center justify-center">
            ...
            <h1 class="h6">
              <!-- {{ Number(order.amount.pesos / order.amount.tasa).toFixed(2) }} Bs -->
              {{ $currencyFormat($unFormat(`${order.amount.pesos}`) / order.amount.tasa) }} Bs
            </h1>
            <p>
              <!-- <span class="primary--text"
                >{{ order.amount.pesos.format(0, 3, '.') }} $</span
              > -->
              <span class="primary--text"
                >{{ Number($unFormat(`${order.amount.pesos}`)).format(0, 3, '.') }} $</span
              >
          ...
          <div class="_section mt-5 b-shadow purple white--text">
            <div class="overline">Punto de Venta</div>
          ...
          <div class="_section _top b-shadow primary white--text">
            <div class="overline">Remitente</div>
            <div class="d-flex flex-column mt-3 text-capitalize">
              <span>
                {{
                  `${order.remitente.name} ${
                    order.remitente.nameSecond ? order.remitente.nameSecond : ''
                  } ${order.remitente.surname} ${
                    order.remitente.surnameSecond
                      ? order.remitente.surnameSecond
                      : ''
                  }`
                }}
              ...
              <p>
                <span>{{
                  `${$getId(order.remitente.tid)}${order.remitente.id}`
                }}</span>
          ...
          <div class="_section _top b-shadow blue white--text">
            <div class="overline">destinatario</div>
            <div class="d-flex flex-column mt-3 text-capitalize">
              <span>
              ...
              <p>
                <span>{{
                  `${$getId(order.destinatario.tid)}${order.destinatario.id}`
                }}</span>
                <span class="mx-2">|</span>
          ...
          <!-- payment method -->
          <div class="_section _top b-shadow yellow black--text">
            <div class="overline">metodo de pago</div>
            <div class="d-flex flex-column mt-3 text-capitalize">
              <span v-if="order.payment_method && order.payment_method.type">
                {{ order.payment_method.type }}
              </span>
              <span v-if="order.payment_method && order.payment_method.text">
          ...
          <div class="_section border-r _top white b-shadow black--text">
            ...
            <div v-if="resume">
              <div
                class="d-flex flex-column flex-lg-row justify-lg-space-between mt-5 text-capitalize"
              >
                <v-btn
                  depressed
                  color="pink"
              ...
              <div class="mt-5">
                ...
                <div v-if="control.cancel">
                  ...
                  />
                  <v-btn
                    :disabled="cancelMessage.length < 5"
                    color="pink"
                    class="white--text"
                    block
                    @click="orderCancel"
            ...
            <div v-else>
              <div class="d-flex flex-column mt-3">
                ...
                <span v-if="order.status !== 'cancelada'">
                  <strong class="mr-1">Porcentaje: </strong>
                ...
                <p v-if="order.status !== 'pendiente'">
                  ...
                  <span class="text-capitalize">
                    {{
                      $moment(order.payout.timestamp).format(
                        'DD-MM-YYYY hh:mm a'
                      )
                    }}
                  </span>
                ...
                <span v-if="order.status === 'cancelada'">
                  <strong class="mr-1">Motivo cancelacion: </strong>
                  {{ order.payout.message }}
                </span>
                <v-btn
...
export default {
  ...
  methods: {
    ...
    async sendEmail(email, url) {
      try {
        await axios.post(
          'https://us-central1-giros-venezuela-9d086.cloudfunctions.net/sendemail',
          {
            to: email,
    ...
    calcPercentage(order) {
      const a = (
        (order.amount.pesos * this.findUserPoint(order.uid).percentage) /
        100
      ).format(0, 3, '.')
