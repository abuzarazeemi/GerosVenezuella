<template>
  <v-card class="pa-6 b-shadow">
    ...
    <!-- <div
      v-if="
        !showOrders ||
        showOrders.filter((e) => e.status === 'pendiente').length === 0
      "
...
export default {
  ...
  computed: {
    ...mapGetters('adminOrders', ['getPendingOrders']),
  ...
  methods: {
    async copyTextClipboard(o) {
      ...
      const copy = `========================\nPUNTO DE VENTA\nNombre: ${
        ...
        r.name
      } ${r.nameSecond ? r.nameSecond : ''} ${r.surname} ${
      ...
      \nEnvio: ${Number(this.$unFormat(`${a.amount.pesos}`)).format(
        0,
        3,
        '.'
